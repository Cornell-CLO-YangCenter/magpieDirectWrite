...
#define SYS_CONFIG_AUDIO_FILE_LEN_IN_SECONDS (10)  //900
...
static uint32_t const  SYS_CONFIG_SAMPLE_RATE = AUDIO_SAMPLE_RATE_384kHz;
...
static uint32_t const  SYS_CONFIG_NUM_BIT_DEPTH = AUDIO_BIT_DEPTH_24_BITS_PER_SAMPLE;
...
// const uint32_t SYS_CONFIG_NUM_CHANNEL_VARIATIONS_TO_TEST = sizeof(sys_num_channel_variations_to_test) / sizeof(sys_num_channel_variations_to_test)[0];
 static uint32_t const SYS_CONFIG_NUM_CHANNEL = WAVE_HEADER_MONO;


// in mono mode, we only record one channel, choose it here
static const Audio_Channel_t SYS_CONFIG_CHANNEL_TO_USE_FOR_MONO_MODE = AUDIO_CHANNEL_0;
...
// derived quantities, don't touch
// smpls/sec / samples/dma_blocks = dma_blocks/sec, x SYS_CONFIG_AUDIO_FILE_LEN_IN_SECONDS = num_dma_blocks
static uint32_t const SYS_CONFIG_NUM_DMA_BLOCKS = (uint32_t)(SYS_CONFIG_SAMPLE_RATE*SYS_CONFIG_AUDIO_FILE_LEN_IN_SECONDS/8192);
static uint32_t const  SYS_CONFIG_NUM_BYTES_PER_CHANNEL = SYS_CONFIG_NUM_BIT_DEPTH/8;
static uint32_t const SYS_CONFIG_NUM_SECTORS = SYS_CONFIG_NUM_DMA_BLOCKS*(8192/512)*SYS_CONFIG_NUM_BYTES_PER_CHANNEL*SYS_CONFIG_NUM_CHANNEL + 1;// add 1 for the header
static uint32_t const SYS_CONFIG_FILE_LEN_BYTES = SYS_CONFIG_NUM_SECTORS*512;
#endif /* SYS_CONFIG_H_ */

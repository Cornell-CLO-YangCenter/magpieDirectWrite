...
int afe_control_enable(Audio_Channel_t channel)
{
    ...
    if (channel == AUDIO_CHANNEL_0)
    {
        ...
        gpio_write_pin(&bsp_pins_afe_ch0_en_cfg, true); // turns on 5v op-amp supply ch0
        MXC_Delay(MXC_DELAY_MSEC(1000)); // rwa edit, needed for mysterious reason (no start of 5v or CM otherwise)
    }
    else if (channel == AUDIO_CHANNEL_1)
    {
        channel_1_is_enabled = true;
        preamp_1_is_enabled = true;
        gpio_write_pin(&bsp_pins_afe_ch1_en_cfg, true); // turns on 5v op-amp supply ch1
        MXC_Delay(MXC_DELAY_MSEC(1000));// rwa edit, needed for mysterious reason
    ...
    // then power on the MEMs microphones and preamp common-mode via the I2C controlled load switches

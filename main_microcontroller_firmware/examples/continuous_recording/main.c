...
// comment out 1 of the following 2 for blockig versus non-blocking
//#define SDHC_NON_BLOCKING
#define SDHC_BLOCKING




//#include "periphDirectAccess.txt" // rwa debug
...
static void write_wav_file(Wave_Header_Attributes_t *wav_attr);
...
static void start_recording(uint8_t number_of_channel, Audio_Sample_Rate_t rate, Audio_Bits_Per_Sample_t bit, SD_Card_Bank_Card_Slot_t sd_slot);
...
static void sdio_write_cb(mxc_sdhc_cmd_cfg_t *, int);
...
static uint32_t sdio_busy = 0;
static uint32_t chunk_packing_idx = 0; // global because set in SDHC callback
static uint32_t total_num_bytes_in_the_buff = 0; // global because set in SDHC callback
static uint32_t num_dma_blocks_consumed = 0; // global because modified in SDHC callback

/* Public function definitions ---------------------------------------------------------------------------------------*/
int main(void)
{

    volatile uint32_t debugRWA1=0,debugRWA2=0;
...
void write_wav_file(Wave_Header_Attributes_t *wav_attr)
{
    volatile uint32_t debugRWA1_wave=0,debugRWA2_wave=0;
    static DWORD lba_current;
    printf(
    ...
    lba_current = lba+1;// each lba is 512 bytes. We start at 1 instead of 0 so we don't overwrite the WAV header.
    ...
    if (sd_card_fopen_writeHeader_preErase(file_name_buff, POSIX_FILE_MODE_WRITE) != E_NO_ERROR)
    {
...
// we will be writing to absolute LBA sector numbers, so it doesn't matter where the file pointer currently is (I hope)

    // there will be some integer truncation here, good enough for this early demo, but improve file-len code eventually
    const uint32_t num_dma_blocks_in_the_file = SYS_CONFIG_NUM_DMA_BLOCKS;
...
#define FULL_BYTE_POOL_SIZE (MAX_WORKSPACE_NEEDED_FOR_ENDIAN_SWAP_AND_Q31_EXPANSION * 3)
    ...
    uint32_t num_sectors_in_the_buff = 0;
    ...
    // // rwa debug
    // debugRWA1_wave = SPI1_CTRL2_direct;
    // debugRWA2_wave = 10;
    ...
    // moved to global (used in callback)
    ...
    {        
        ...
        // the following function returns "num_buffer_chunks_with_data_to_be_consumed_ch0" or ch1
        // these values are incremented for every ISR call and decremented on every audio_dma_consume_buffer call
        if (audio_dma_num_buffers_available(AUDIO_CHANNEL_0) > 0 && audio_dma_num_buffers_available(AUDIO_CHANNEL_1) > 0)
        {
            ...
            num_sectors_in_the_buff = total_num_bytes_in_the_buff >> 9; // div by 512

            if (chunk_packing_idx >= num_processed_chunks_we_can_fit_in_the_workspace_buff && sdio_busy==0) // we filled up the chunk-packing sector as much as we can
            {


                sdio_busy = 1; // set back to 0 in the callback
                sdio_write_sectors(lba_current, start_of_chunk_packing_sector,num_sectors_in_the_buff) ; // non-blocking, with callback
                lba_current += num_sectors_in_the_buff;

                #ifdef SDHC_BLOCKING //blocking code
                while(sdio_busy) {}; // wait for callback to set sdio_busy low. Blocking behavior
                // if non-blocking, the following 3 lines are executed in the callback
                chunk_packing_idx = 0;
    ...
    if (total_num_bytes_in_the_buff > 0 && sdio_busy==0)
    {
        num_sectors_in_the_buff = total_num_bytes_in_the_buff >> 9; // div by 512
        sdio_busy = 1;
        sdio_write_sectors(lba_current, start_of_chunk_packing_sector,num_sectors_in_the_buff) ;
        lba_current += num_sectors_in_the_buff; // increment the lba to point at the next available unwritten sector
        while(sdio_busy) {}; // wait for callback to set sdio_busy low. Blocking, not worth worrying about non-blocking since it only happens once
    ...
    // we already wrote the header at file creation time
...
int sdio_write_sectors(uint32_t start_lba, const uint8_t *buf,uint32_t BLOCKS_PER_WRITE) {
    ...
    // note, addume FATFS has already set the control reg for 4-bit I/O
    #define SECTOR_SIZE 512 // bytes
    //#define BLOCKS_PER_WRITE 24*1024/SECTOR_SIZE // mono 24-bit , EDIT for others!
    //#define TIMEOUT_MS 1000
    mxc_sdhc_cmd_cfg_t sdhc_req; // struct for sending command to the SD card
    int err;

    memset(&sdhc_req, 0, sizeof(mxc_sdhc_cmd_cfg_t)); // clear the struct

    // don't change the SD card control reg; it was set by FATFS when the file was opened
    // re-write every time. This contains the hardware transaction width, set to 4 bits (somewhere ...)
    sdhc_req.host_control_1 = SD_host_control_reg; // saved from when FATFS opened the file, just re-write every time

    sdhc_req.command = MXC_SDHC_LIB_CMD25;  // CMD25 = WRITE_MULTIPLE_BLOCK, also contains oter control fields for response type, etc.

    sdhc_req.arg_1 = start_lba;         // destination starting block

    sdhc_req.sdma = (uint8_t *)buf;   // DMA source

    sdhc_req.block_size = SECTOR_SIZE;

    sdhc_req.block_count = BLOCKS_PER_WRITE;  // 48 for a single 24kB DMA buffer (24 bits mono 384k)

    sdhc_req.direction = MXC_SDHC_DIRECTION_WRITE;

    sdhc_req.callback = sdio_write_cb; // non-blocking transaction, will call the callback function when complete.
    // note that the sdio dma is reading from the array
    //  that is passed in as the input, so don't change it (at least within the block count limit) while its executing

    sdhc_req.dma = 1; // yes, use dma callback 

    err = MXC_SDHC_SendCommand(&sdhc_req);  // starts SDHC DMA transfer, will go to the callback function when done. This is non-blocking
...
// callback for completion of SD card write operation
static void sdio_write_cb(mxc_sdhc_cmd_cfg_t * req, int error)
{
sdio_busy = 0;
#ifdef SDHC_NON_BLOCKING
chunk_packing_idx = 0;
total_num_bytes_in_the_buff = 0;
num_dma_blocks_consumed += 1;
#endif
...
static void initialize_system(void)
{
     ...
     volatile uint32_t debugRWA1=0,debugRWA2=0; // rwa debug
    ...
    hspi == bsp_spi_adc_ch0_data_spi_handle;
    debugRWA1 = hspi->ctrl2; // rwa debug
    debugRWA2 = 10;
...
static void start_recording(uint8_t number_of_channel, 
    Audio_Sample_Rate_t rate, Audio_Bits_Per_Sample_t bit, 
                            SD_Card_Bank_Card_Slot_t sd_slot)
{
